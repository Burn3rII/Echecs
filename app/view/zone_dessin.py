"""Classes implémentant les zones de dessin. Une fenêtre doit être passée
en argument.
Ces classes créent le canvas et gèrent son contenu.
Deux classes sont possibles : la zone de dessin qui gère le canvas
principal et la zone de dessin promotion qui gère le canvas de
promotion.
"""
...
# ----- Paramètres de jeu
from .constantes import (NB_COLONNES, NB_LIGNES, LARGEUR, HAUTEUR,
                         LARGEUR_PROMOTION, HAUTEUR_PROMOTION,
                         NB_TYPES_PIECES, NB_LIGNES_PROMOTION)
...
class ZoneDessin:
    ...
    def __init__(self, window):
        ...
        # Zone de dessin
        self.canvas = tk.Canvas(self.window, bg="white", width=LARGEUR,
                                height=HAUTEUR)
        self.canvas.grid(row=0, column=0, columnspan=2)
        self.canvas.update()  # .update() met à jour l'interface
        # utilisateur avec les modifications apportées précédemment.
        # Sans ça, le calcul de la taille du canvas est fait plus tard,
        # en lançant mainloop donc en utilisant
        # main_canvas.winfo_width(), on obtient 1 et pas la taille
        # réelle.
    ...
    def dessiner_piece(self, piece, case_x, case_y, couleur):
        ...
        """
        classe = globals().get(piece)  # Transformation de la str en nom
        # de classe.
        if classe is not None and issubclass(classe, Piece):
            self.pieces[case_x][case_y] = classe(
                self.canvas, NB_COLONNES, NB_LIGNES, case_x, case_y,
                couleur)
        else:
    ...
    def deplacer_piece(self, case_x_avant, case_y_avant, case_x_apres,
        ...
        if self.pieces[case_x_apres][case_y_apres] is None:
            self.pieces[case_x_avant][case_y_avant].deplacer_piece(
                NB_COLONNES, NB_LIGNES, case_x_apres, case_y_apres)
            self.pieces[case_x_apres][case_y_apres] = \
                self.pieces[case_x_avant][case_y_avant]
            self.pieces[case_x_avant][case_y_avant] = None
        else:
            raise ValueError("Déplacement d'une pièce vers un emplacement "
                             "contenant déjà une pièce.")

    def reset(self):
        """Réinitialise la zone de dessin.
        """
        # Pièces
...
class ZoneDessinPromotion:
    ...
    def dessiner_piece(self, piece, case_x, case_y, couleur):
        ...
        la classe, par exemple "Pion" pour un pion.
        """
        classe = globals().get(piece)  # Transformation de la str en
        # nom de classe.
        if classe is not None and issubclass(classe, Piece):
            self.pieces[case_x] = classe(self.canvas, NB_TYPES_PIECES,
                                         NB_LIGNES_PROMOTION, case_x,
                                         case_y, couleur)
        else:
            raise ValueError("Classe de la pièce à créer invalide")

    def dessiner_pieces(self, couleur):
        """Dessine toutes les pièces en lesquelles peut être promu un
        pion dans les cases.
        """
